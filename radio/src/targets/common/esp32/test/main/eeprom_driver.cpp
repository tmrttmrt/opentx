...
const char * const eepromFname = "/spf/foff";
...
//    return;
    if (stat(eepromFname, &st) == 0) {
        ...
        if(st.st_size<OTX_EEPROM_SIZE){
            
            char buff[1]={(char)0};
            size_t bw = fwrite ((void *)buff, 1 , OTX_EEPROM_SIZE, fp);
            if (bw!=OTX_EEPROM_SIZE) { 
                ESP_LOGE(TAG,"Failed to allocate %d bytes for ' %s'.",OTX_EEPROM_SIZE, eepromFname);
            }
            fclose(fp);
            if (stat(eepromFname, &st) == 0){
